...
/*
// cheap hack to debug alloc errors
// also in mongo.c
...
inline bson* bson_init( bson * b , char * data , bson_bool_t mine ){
    b->data = data;
    b->owned = mine;
    return b;
}
...
MONGO_INLINE void bson_append_byte( bson_buffer * b , char c ){
    b->cur[0] = c;
    b->cur++;
}
MONGO_INLINE void bson_append( bson_buffer * b , const void * data , int len ){
    memcpy( b->cur , data , len );
    b->cur += len;
}
MONGO_INLINE void bson_append32(bson_buffer * b, const void * data){
    bson_little_endian32(b->cur, data);
    b->cur += 4;
}
MONGO_INLINE void bson_append64(bson_buffer * b, const void * data){
...
bson_buffer * bson_append_string_base( bson_buffer * b , const char * name , const char * value, int value_len , bson_type type){
    if ( ! bson_append_estart( b , type , name , 4 + value_len + 1 ) ) return 0;
    int sl = value_len + 1;
    bson_append32( b , &sl);
    bson_append( b , value , value_len );
    bson_append_byte(b, 0);
    return b;
}
...
bson_buffer * bson_append_substr( bson_buffer * b , const char * name , const char * value, int value_len ){
    return bson_append_string_base(b, name, value, value_len, bson_string);
}
